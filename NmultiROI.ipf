...
if (ROIListSelWave[ROIList] == 1)
	...
	if(!DataFolderExists("root:ROI_analysis"))
		NewDataFolder/S root:ROI_analsis
	endif
	...
	wave roiX = $("root:twoP_ROIS:" + ROI+"_x") // these 2 waves describe a profile over which to
	...
	wave inputWave = $("root:twoP_Scans:" +ScanName + ":" + ScanName + "_" + channel)  	
	...
	for (i = 0; i < numtrials; i +=1)
		for (j = 0; j < batchsize; j+=1)
			...
			if(waveexists(inputWave) == 0)
				abort "root:twoP_Scans:" +  ScanName + ":" +  ScanName + "_" + channel + " - does not exist"
			else
			...
			if(smthMode)
					Smooth/S=2 21, ROIavg //Savitzky-Golay smoothing // SS 10AUG2017 changed num from 15 to 9
			endif
			...
			if(V_flag ==0) //if does not window exist
				...
				TextBox/C/N=directions/A=MT " {" + num2str(direction[0]) +"," + num2str(direction[1]) +"," + num2str(direction[2]) +"," + num2str(direction[3]) +"," + num2str(direction[4])+"," + num2str(direction[5]) +"," + num2str(direction[6]) +"," + num2str(direction[7]) + "}" // Create textbox with order of directions
			else
